    ...
    <section id="departments" className="py-20 lg:py-28 bg-[#F8FAFC] overflow-hidden scroll-mt-20">
      <div className="container-custom">
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-3.5 rounded-2xl border text-center transition-all duration-200 group relative ${
                    isActive
                      ? 'bg-navy-950 text-white border-gold-500 shadow-lg scale-105 z-10'
                      : 'bg-white text-navy-900 border-slate-200 hover:border-gold-400 hover:bg-slate-50'
                  }`}
                >
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 transition-colors ${
                      isActive
                        ...
                        : 'bg-slate-100 text-navy-800 group-hover:bg-navy-900 group-hover:text-gold-400'
                    }`}
                  >
                  ...
                  <span
                    className={`text-[10px] mt-1 font-medium ${
                      isActive ? 'text-gold-300 font-semibold' : 'text-slate-500'
                    }`}
                  >
                    {cat.countLabel}
                  </span>
        ...
        <Reveal direction="fade" delay={150}>
          <div className="mb-10 p-6 md:p-8 rounded-3xl bg-gradient-to-r from-navy-950 via-navy-900 to-navy-850 text-white border border-navy-750 shadow-xl flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          ...
          {selectedCategory.courses.map((course: CourseItem, idx: number) => {
            ...
            return (
              <Reveal
                ...
                delay={(idx % 6) * 70}
              >
                <div className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-card hover:shadow-card-hover transition-all duration-300 flex flex-col justify-between group h-full">
                  <div>
                    ...
                    <div className="relative h-48 w-full overflow-hidden bg-navy-950">
                      ...
                      <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-10">
                        <div className="w-9 h-9 rounded-xl bg-white/90 backdrop-blur-md text-navy-900 flex items-center justify-center shadow-md">
                      ...
                      <div className="absolute bottom-3 left-4 right-4 z-10 flex items-center justify-between text-xs">
                        ...
                        {course.seats && (
                          <span className="text-[10px] text-white font-bold px-2 py-0.5 rounded bg-blue-600/80 backdrop-blur-sm">
                            {course.seats}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-6">
                      ...
                      {course.affiliations && (
                        <div className="text-[11px] text-blue-800 bg-blue-50 border border-blue-200 rounded-lg p-2 mb-3 leading-tight">
                      ...
                      {/* Specializations list if present (e.g. M.Pharm, Physiotherapy departments, M.Sc Nursing seats) */}
